...
import { PreviewModal } from "~/features/sharing/components/PreviewModal";
import { Button } from "~/shared/components/button";
import { VideoProcessingHelp } from "~/features/video/components/VideoProcessingHelp";
import { createRetryAction, handleVideoError } from "~/features/video/services/utils/error-handler";
import { extractVideoMetadata } from "~/features/video/services/utils/metadata";

import { compressAudioFile, getFileSizeMB, isFileTooLarge } from "~/features/video/services/utils/compression";
import type { ParsedTranscription } from "~/features/video/services/utils/transcription-parser";
...
function InnerCanvas({ 
  ...
  // Convex queries
  const canvasState = useQuery(api.domains.projects.canvas.getCanvasState, { projectId });
  const projectVideos = useQuery(api.domains.videos.videos.getVideosByProject, { projectId });
  const projectAgents = useQuery(api.domains.ai.agents.getAgentsByProject, { projectId });
  const userProfile = useQuery(api.domains.users.profiles.getUserProfile);
  
  // Convex mutations
  const createVideo = useMutation(api.domains.videos.videos.createVideo);
  const updateVideo = useMutation(api.domains.videos.videos.updateVideo);
  const generateUploadUrl = useMutation(api.domains.shared.files.generateUploadUrl);
  const updateVideoStorage = useMutation(api.domains.videos.videos.updateVideoStorage);
  const createAgent = useMutation(api.domains.ai.agents.createAgent);
  const updateAgentDraft = useMutation(api.domains.ai.agents.updateAgentDraft);
  const updateAgentConnections = useMutation(api.domains.ai.agents.updateAgentConnections);
  const updateAgentPosition = useMutation(api.domains.ai.agents.updateAgentPosition);
  const saveCanvasState = useMutation(api.domains.projects.canvas.saveCanvasState);
  const scheduleTranscription = useMutation(api.domains.videos.videoJobs.scheduleTranscription);
  const deleteVideo = useMutation(api.domains.videos.videos.deleteVideo);
  const deleteAgent = useMutation(api.domains.ai.agents.deleteAgent);
  const createShareLink = useMutation(api.domains.shared.shares.createShareLink);
  const userShares = useQuery(api.domains.shared.shares.getUserShares);
  const uploadManualTranscription = useMutation(api.domains.videos.transcriptionMutations.uploadManualTranscription);
  const updateTranscriptionStatus = useMutation(api.domains.videos.transcriptionMutations.updateTranscriptionStatus);
  const generateTranscriptionUploadUrl = useMutation(api.domains.shared.files.generateUploadUrl);
  const videoForTranscription = useQuery(api.domains.videos.videos.getVideo, transcriptionVideoId ? { id: transcriptionVideoId } : "skip");


  
  // Convex actions for AI
  const generateContent = useAction(api.domains.ai.generation.generateContent);
  const generateThumbnail = useAction(api.domains.ai.thumbnail.generateThumbnail);
  const refineContent = useAction(api.domains.ai.generation.refineContent);
  ...
  const handleGenerate = useCallback(async (nodeId: string, thumbnailImages?: File[], additionalContext?: string) => {
    ...
    try {
      ...
      if (agentNode.data.type === "thumbnail" && thumbnailImages) {
        ...
        console.log("[Canvas] Concept received:", thumbnailResult.substring(0, 100) + "...");
        
        result = thumbnailResult;
        // Note: thumbnailUrl and thumbnailStorageId are not available from current thumbnail API
        ...
        // Note: prompt info not available from current generation API
      }
  ...
  const handleChatMessage = useCallback(async (message: string) => {
    ...
    try {
      ...
      // Prepare profile data for API calls
      const profileData = userProfile ? {
        channelName: userProfile.channelName,
        contentType: userProfile.contentType,
        niche: userProfile.niche,
      ...
      if (agentNode.data.type === "thumbnail" && agentNode.data.thumbnailUrl) {
        ...
        // Use refineContent for thumbnail refinement since refineThumbnail doesn't exist
      ...
      for (const node of nodes) {
        try {
          if (node.type === 'video' && node.data.videoId) {
            ...
            // Note: deleteTranscription API not available
  ...
  const handleVideoUpload = async (file: File, position: { x: number; y: number }, retryCount = 0) => {
    ...
    try {
      ...
      // The actual URL will be set when updateVideoStorage completes and projectVideos refreshes
      ...
      (async () => {
        try {
          ...
          // Note: updateVideoMetadata not available in current API
  ...
  const handleTranscriptionUploadComplete = useCallback(async (transcription: ParsedTranscription, file: File) => {
    ...
    try {
      ...
      // createTranscription API not available
      console.log("[Canvas] createTranscription functionality not implemented");
      
      let transcriptionId;
      try {
        console.log("[Canvas] createTranscription not available");
  ...
  }, [transcriptionUploadVideoId, uploadManualTranscription, generateTranscriptionUploadUrl, setNodes, setEdges, nodes, handleViewTranscription, projectId]);
  ...
  useEffect(() => {
    ...
    if (!hasLoadedFromDB && projectVideos !== undefined && projectAgents !== undefined) {
      ...
      setNodes([...videoNodes, ...agentNodes]); // ...transcriptionNodes removed
      ...
      console.log("[Canvas] Finished loading from DB, total nodes:", [...videoNodes, ...agentNodes].length);
    }
  }, [projectVideos, projectAgents, hasLoadedFromDB, setNodes, setEdges, handleGenerate, handleChatButtonClick, handleViewTranscription, retryTranscription, handleManualTranscriptionUpload, handleVideoClick, handleRegenerateClick]);
                  ...
                  <TranscriptionUpload
                    ...
                    onUploadComplete={async (transcription: ParsedTranscription, file: File) => {
                      try {
                        ...
                        toast.success(`Transcription file "${file.name}" processed!`);
          ...
          <ReactFlow
            ...
            onNodeDragStop={async (_event: any, node: any) => {
              ...
              if (node.type === 'video' && node.data.videoId) {
                ...
                try {
                  // await updateTranscriptionPosition({ // API not available
