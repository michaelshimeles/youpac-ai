...
import { useProjectStore } from "~/features/dashboard/services/store/useProjectStore";
...
export function useProjectOperations() {
  ...
  // Queries
  const allProjects = useQuery(api.domains.projects.projects.list, {});
  const createProjectMutation = useMutation(api.domains.projects.projects.create);
  const updateProjectMutation = useMutation(api.domains.projects.projects.update);
  const deleteProjectMutation = useMutation(api.domains.projects.projects.remove);
...
export function useProjectSettings(projectId?: Id<"projects">) {
  const project = useQuery(api.domains.projects.projects.get, projectId ? { id: projectId } : "skip");
  const updateProjectMutation = useMutation(api.domains.projects.projects.update);
...
export function useProjectCollaboration(projectId: Id<"projects">) {
  const project = useQuery(api.domains.projects.projects.get, { id: projectId });
  const shareProjectMutation = useMutation(api.domains.projects.projects.share);
  const addCollaboratorMutation = useMutation(api.domains.projects.projects.addCollaborator);
  const removeCollaboratorMutation = useMutation(api.domains.projects.projects.removeCollaborator);
...
export function useProjectAnalytics(projectId: Id<"projects">) {
  const projectVideos = useQuery(api.domains.videos.videos.listByProject, { projectId });
  const projectAgents = useQuery(api.domains.ai.agents.getByProject, { projectId });
