...
import { useTranscriptionService } from "~/shared/services/ServiceProvider";
import { TranscriptionOptions, TranscriptionResult } from "~/shared/types/video";
...
export function useTranscription() {
  ...
  const transcribeAction = useAction(api.domains.videos.transcriptionActions.transcribeVideo);
  const updateStatusMutation = useMutation(api.domains.videos.transcriptionMutations.updateTranscriptionStatus);
...
export function useManualTranscription() {
  const updateVideoMutation = useMutation(api.domains.videos.transcriptionMutations.updateVideoTranscription);
...
export function useTranscriptionEditor(videoId: Id<"videos">) {
  const video = useQuery(api.domains.videos.transcriptionMutations.getVideoWithTranscription, { videoId });
  const updateVideoMutation = useMutation(api.domains.videos.transcriptionMutations.updateVideoTranscription);
  const clearTranscriptionMutation = useMutation(api.domains.videos.videos.updateVideo);
