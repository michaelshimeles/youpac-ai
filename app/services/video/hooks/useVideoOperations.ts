...
import { useVideoService } from "~/services/ServiceProvider";
import { useCanvasStore } from "~/services/canvas/store/useCanvasStore";
import { VideoUploadOptions } from "~/types/video";
...
export function useVideoOperations(projectId: Id<"projects">) {
  ...
  // Mutations
  const updateVideoMutation = useMutation(api.videos.update);
  const deleteVideoMutation = useMutation(api.videos.remove);
...
export function useVideoProcessing(videoId?: Id<"videos">) {
  const video = useQuery(api.videos.get, videoId ? { id: videoId } : "skip");
