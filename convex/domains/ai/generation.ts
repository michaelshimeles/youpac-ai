...
export const generateContent = action({
  ...
  handler: async (ctx, args): Promise<string> => {
    ...
    // Get agent details
    const agent = await ctx.runQuery(api.domains.ai.agents.getAgent, { id: args.agentId });
    if (!agent) throw new Error("Agent not found");

    // Update status to generating
    await ctx.runMutation(api.domains.ai.agents.updateAgentDraft, {
      id: args.agentId,
      draft: agent.draft,
      status: "generating",
    });
...
export const refineContent = action({
  ...
  handler: async (ctx, args): Promise<string> => {
    ...
    await ctx.runMutation(api.domains.ai.agents.addAgentChatMessage, {
      id: args.agentId,
      role: "user",
