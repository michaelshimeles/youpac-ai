...
export const scheduleTranscription = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, internal.domains.videos.videoJobs.transcribeInBackground, args);
...
export const transcribeInBackground = internalAction({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; transcription: string; service?: string } | void> => {
    ...
    try {
      ...
      const result = await ctx.runAction(api.domains.videos.transcriptionActions.transcribeWithElevenLabs, {
      ...
      // Mark as failed
      await ctx.runMutation(internal.domains.videos.videoJobs.markTranscriptionFailed, {
        videoId: args.videoId,
...
export const scheduleThumbnailGeneration = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, internal.domains.videos.videoJobs.generateThumbnailInBackground, {
      videoId: args.videoId,
...
export const generateThumbnailInBackground = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runAction(api.domains.ai.thumbnail.generateThumbnail, {
